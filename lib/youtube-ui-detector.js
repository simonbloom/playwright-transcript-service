...
class YouTubeUIDetector {
  ...
  async findTranscriptButton(page, options = {}) {
    ...
    const strategies = [
      this.findInDescription.bind(this), // Try description first - common in 2025
      this.findInMenu.bind(this), // Then try menu
  ...
  async findInDescription(page) {
    console.log('Trying description strategy for transcript button...');
    
    try {
      // First, scroll down to find the description area
      const descriptionContainer = await page.$('#description, ytd-video-secondary-info-renderer');
      if (!descriptionContainer) {
        console.log('No description container found');
        return null;
      }
      
      // Scroll the description into view
      await descriptionContainer.scrollIntoViewIfNeeded();
      await page.waitForTimeout(500);
      
      // Find and click the expand button ("...more")
      let expandButton = null;
      for (const expandSelector of this.transcriptSelectors.description) {
        expandButton = await page.$(expandSelector);
      ...
      if (!expandButton) {
        console.log('No expand button found in description');
        return null;
      }
      
      // Click the expand button
      await expandButton.click();
      console.log('Clicked expand button, waiting for description to expand...');
      await page.waitForTimeout(1500);
      
      // Now look for transcript button within the expanded description
      // Get the expanded description container
      const expandedDescription = await page.$('#description-inner, #description, ytd-text-inline-expander');
      if (!expandedDescription) {
        console.log('No expanded description found');
        return null;
      }
      
      // Search for transcript button within the description
      ...
      for (const selector of transcriptSelectors) {
        try {
          // Look for the button within the description container
          const transcriptButton = await expandedDescription.$(selector);
          if (transcriptButton && await transcriptButton.isVisible()) {
            console.log(`Found transcript button in description with selector: ${selector}`);
      ...
      // Alternative: Search by evaluating in page context
      const transcriptElement = await expandedDescription.evaluateHandle(container => {
        // Look for any clickable element with "Show transcript" text
        const walker = document.createTreeWalker(
          ...
          NodeFilter.SHOW_ELEMENT,
          {
            acceptNode: (node) => {
              ...
              const isClickable = node.tagName === 'BUTTON' || 
                                 node.tagName === 'A' || 
                                 node.getAttribute('role') === 'button' ||
                                 node.classList.contains('yt-formatted-string');
              return text.toLowerCase().includes('show transcript') && isClickable
      ...
      if (transcriptElement && await transcriptElement.asElement()) {
        console.log('Found transcript element via tree walker');
        const element = await transcriptElement.asElement();
        await element.scrollIntoViewIfNeeded();
        await page.waitForTimeout(500);
        return element;
      }
      
      console.log('No transcript button found in expanded description');
      
    } catch (error) {
      console.log('Error in findInDescription:', error.message);
    }
